...
#            print "zoomFactor:",zoomFactor
            ...
            coordsInDisplayedImage = np.array([x,y])
            if np.any(coordsInDisplayedImage < 0) or np.any(coordsInDisplayedImage > self.panAndZoomState.shape[:2]):
                ...
                print( "this pixel holds ",self.img[coordsInFullImage[1],coordsInFullImage[0]])
                if self.onLeftClickFunction is not None:
                    self.onLeftClickFunction(coordsInFullImage[1],coordsInFullImage[0])
